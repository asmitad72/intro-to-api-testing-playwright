...
test('update order status with invalid ID, should received an error', async ({ request }) => {
  // prepare request body
  const requestBody = {
    status: 'OPEN',
  }
  // Send a POST request to the server
  const response = await request.put('https://backend.tallinn-learning.ee/test-orders/-5/status', {
    data: requestBody,
  })
  // Log the response status and body
  console.log('response status:', response.status())
...
test('get test order time without headers, should receive an error', async ({ request }) => {
